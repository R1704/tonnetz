...
class MarkovProgression(ProgressionAlgo):
    ...
    def __init__(self, order: int = 1, **kwargs):
        ...
        super().__init__()
        self.order = order
        self.transition_matrix: Dict[Tuple[str, ...], Dict[str, float]] = defaultdict(
    ...
    def train(
        self, progressions: List[List[str]], weights: Optional[List[float]] = None
    ):
        """
        ...
        for _ in range(length - 1):
            ...
            if state_key in self.transition_matrix:
                ...
                candidates = self._get_fallback_candidates(
                    current_state[-1] if current_state else str(start_chord)
                )
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            if len(self.transition_matrix) < 50
            else "too_large",
        }
...
def _apply_temperature(
    probabilities: Dict[str, float], temperature: float = 1.0
) -> Dict[str, float]:
