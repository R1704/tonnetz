...
class SearchBasedProgression(ProgressionAlgo):
    ...
    def _beam_search(self, start_chord: Chord, length: int) -> List[Chord]:
        ...
        for step in range(length - 1):
            ...
            beam = heapq.nlargest(
                self.beam_width, new_beam, key=lambda s: s.score + s.heuristic
            )
    ...
    def _astar_search(self, start_chord: Chord, length: int) -> List[Chord]:
        ...
        open_set = [
            SearchState([start_chord], 0.0, self._heuristic([start_chord], length))
        ]
    ...
    def _evaluate_neo_riemannian_efficiency(self, progression: List[Chord]) -> float:
        ...
        for i in range(len(progression) - 1):
            ...
            if (
                self.plr_group.parallel(current) == next_chord
                or self.plr_group.leading_tone_exchange(current) == next_chord
                or self.plr_group.relative(current) == next_chord
            ):
                plr_moves += 1
