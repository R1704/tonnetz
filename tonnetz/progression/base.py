...
class CompositeProgressionAlgo(ProgressionAlgo):
    ...
    def __init__(
        self, algorithms: List[ProgressionAlgo], strategy: str = "sequential"
    ) -> None:
        """
    ...
    def _generate_sequential(
        self, start: Chord, length: int, **kwargs: Any
    ) -> List[Chord]:
        ...
        for i, algo in enumerate(self.algorithms):
            ...
            segment_length = (
                min(chords_per_algo, remaining) + 1
            )  # +1 for starting chord
...
class ProgressionAnalyzer:
    ...
    def analyze_neo_riemannian_moves(progression: List[Chord]) -> Dict[str, Any]:
        ...
        plr_efficiency = (
            sum(trans_lengths) / len(trans_lengths) if trans_lengths else 0.0
        )

        return {
            "transformations": transformations,
            "plr_efficiency": plr_efficiency,
            "direct_plr_moves": len(
                [t for t in transformations if len(t["transformation"]) == 1]
            ),
        }
