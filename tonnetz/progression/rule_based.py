...
class RuleBasedProgression(ProgressionAlgo):
    ...
    def _setup_harmonic_functions(self) -> None:
        ...
        self.tonic_chords = [
            deg
            for deg, info in self.scale_degrees.items()
            if info["function"] == "tonic"
        ]
        self.predominant_chords = [
            deg
            for deg, info in self.scale_degrees.items()
            if info["function"] == "predominant"
        ]
        self.dominant_chords = [
            deg
            for deg, info in self.scale_degrees.items()
    ...
    def _select_chord_by_function(
        self, target_function: str, previous_chord: Optional[Chord] = None
    ) -> Chord:
        """
    ...
    def _apply_pattern(
        self, pattern_name: str, length: int, start: Chord
    ) -> List[Chord]:
        """
    ...
    def _generate_functional_progression(
        self, start: Chord, length: int
    ) -> List[Chord]:
        """
        ...
        for i in range(1, length):
            ...
            next_chord = self._select_chord_by_function(
                selected_function, progression[-1]
            )
