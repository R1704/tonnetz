...
@click.option('--start-chord', default='C', help='Starting chord (e.g., "C", "Am", "F#maj7")')
@click.option('--algorithm', default='rule_based', 
...
@click.option('--neighborhood', default='moore', 
              type=click.Choice(['moore', 'von_neumann']),
              help='Neighborhood type for cellular automaton')
@click.option('--update-mode', default='synchronous',
              type=click.Choice(['synchronous', 'asynchronous']),
              help='Update mode for cellular automaton')
@click.option('--verbose', '-v', is_flag=True, help='Verbose output')
def simulate(config: Optional[str], width: int, height: int, steps: int,
             start_chord: str, algorithm: str, pattern: Optional[str],
             key: int, mode: str, output: Optional[str], 
             save_animation: Optional[str], neighborhood: str,
             update_mode: str, verbose: bool):
    ...
    algo_params = {
        ...
        'pattern': params.get('progression', {}).get('params', {}).get('pattern', pattern),
    ...
    if algo_params['algorithm'] == 'rule_based':
        ...
        progression_algo = MarkovProgression(
            order=algo_params.get('order', 1)
        )
    ...
    if save_animation:
        ...
        for step in range(sim_params['steps']):
            ...
            if verbose:
                ...
                click.echo(f"Step {step + 1}: {stats['chord_changes']} changes, "
...
@click.option('--color-by', default='quality',
...
@click.option('--format', 'img_format', default='png',
...
def visualize(grid_size: str, algorithm: str, pattern: Optional[str],
              color_by: str, output: str, title: Optional[str],
              img_format: str, dpi: int, figsize: str):
    ...
    plotter.plot_grid(grid, title=plot_title, color_by=color_by,
